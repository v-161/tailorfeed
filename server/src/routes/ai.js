...
// @route   GET /api/ai/engagement-analytics
// @desc    Get engagement timing analytics for AI tips
router.get('/engagement-analytics', auth, async (req, res) => {
  try {
    ...
    userPosts.forEach(post => {
      ...
      // Analyze comments timing (comments already have createdAt)
    ...
    // Calculate best engagement hours
    ...
    res.json({
      ...
      engagementAnalytics: {
        totalEngagements,
        bestHours: bestHours.length > 0 ? bestHours : [9, 12, 18], // Default if no data
        engagementDistribution: hourCounts,
        hasEngagementData: totalEngagements > 0
      }
    });
    ...
    res.status(500).json({
      success: false,
      message: 'Server error fetching engagement analytics'
    });
  }
});
